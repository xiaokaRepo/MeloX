...
nonisolated enum QQMusicModifiedDES {
    ...
    private static func compressedKey(
        left: UInt32,
        right: UInt32
    ) -> UInt64 {
        keyCompression.enumerated().reduce(into: UInt64(0)) {
            result, entry in
            result <<= 1
            if entry.offset < 24 {
                result |= UInt64(
                    (left >> UInt32(28 - entry.element)) & 1
                )
            } else {
    ...
    private static func function(
        ...
        for boxIndex in 0..<8 {
            let shift = 42 - boxIndex * 6
            let value = Int((expanded >> UInt64(shift)) & 0x3F)
            let row = ((value & 0x20) >> 4) | (value & 1)
            let column = (value >> 1) & 0x0F
            substituted = (substituted << 4)
                | UInt32(sBoxes[boxIndex][row * 16 + column])
        }
        return UInt32(
            truncatingIfNeeded: permute(
                UInt64(substituted),
                inputWidth: 32,
                table: functionPermutation
            )
        )
    }
    ...
    private static func permute(
        ...
        table.reduce(into: UInt64(0)) { result, position in
            result <<= 1
            result |= (value >> UInt64(inputWidth - position)) & 1
        }
    }
    ...
    private static func wordReversed(_ bytes: [UInt8]) -> [UInt8] {
        ...
        for offset in stride(from: 0, to: bytes.count, by: 4) {
            result.append(
                contentsOf: bytes[offset..<min(offset + 4, bytes.count)].reversed()
            )
        }
        return result
    }
    ...
    private static func uint64(_ bytes: [UInt8]) -> UInt64 {
        bytes.reduce(into: UInt64(0)) { result, byte in
    ...
    private static func bytes(_ value: UInt64) -> [UInt8] {
        (0..<8).map { index in
            UInt8(truncatingIfNeeded: value >> UInt64((7 - index) * 8))
        }
    }
    ...
    private static let finalPermutation = [
        40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31,
        38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29,
        36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27,
        34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25,
    ]
    private static let keyPermutation = [
    ...
    private static let functionPermutation = [
