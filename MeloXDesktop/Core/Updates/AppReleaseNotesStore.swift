...
final class AppReleaseNotesStore {
    private enum Key {
        static let lastPresentedVersion =
            "appReleaseNotes.lastPresentedVersion"
    }

    let currentVersion: String
    let currentReleaseNotes: AppReleaseNotes?
    private(set) var releaseNotesToPresent: AppReleaseNotes?
    ...
    init(
        ...
        let lastPresentedVersion = defaults.string(
            forKey: Key.lastPresentedVersion
        )
        ...
        if let lastPresentedVersion {
            if AppVersion.compare(
                currentVersion,
                to: lastPresentedVersion
            ) == .orderedDescending {
